...
def check_permission(user, method, permission_to):
    ...
    for group in groups:
        ...
        for permission in permissions:
            if (
                Permission.objects.filter(id=permission["permission_id"])
                .filter(codename=required_permission)
                .exists()
            ):
                return True
...
class EmployeesPermission(permissions.BasePermission):
    ...
    def has_permissions(self, request, _view) -> bool | None:
...
class GroupsPermissionsPermission(permissions.BasePermission):
    ...
    def has_permissions(self, request, _view) -> bool | None:
        return check_permission(
            request.user, request.method, permission_to="permission"
        )


class TaskPermission(permissions.BasePermission):
    message = "O funcionário não tem permissão para gerenciar as tarefa de todos os funcionários"
